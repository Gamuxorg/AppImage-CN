...
1. __收集合适的二进制文件__。如果应用程序已经编译完成，则可以使用现有的二进制文件（例如，包含在.tar.gz，deb或rpm存档中的二进制文件）。请注意，二进制文件绝不能在比你的目标发行版更新的版本上编译。换句话说，如果你的目标是Ubuntu 9.10，你不应该使用在Ubuntu 10.04上编译的二进制文件。对于上游项目，编译用于AppImages的特殊构建可能是有利的，尽管这不是必需的。

2. __收集所有依赖的二进制文件__，收集你的目标基本操作系统所没有的那部分。例如，如果你的目标是Ubuntu，Fedora和openSUSE，那么你需要收集你的应用程序运行所需单Ubuntu，Fedora和openSUSE没有的所有库和其他依赖项。

3. __根据你的二进制文件创建一个可用的AppDir__。一个正常的AppImage会在你执行AppRun文件时运行你的应用程序。

4. __把你的AppDir转换成AppImage__。这将你的AppDir的内容压缩成一个自挂载和自执行的文件。

5. __测试你的AppImage__ 在所有你的目标基础操作系统。这是你不应该跳过的重要一步。各种发行版的细微差异使得这是必须的。虽然在大多数情况下可以创建在各种发行版上运行的AppImages，但这不会自动发生，还需要仔细的手动调整。

虽然理论上可以手动完成所有这些步骤，但AppImageKit包含的工具可以大大简化任务。
